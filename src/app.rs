...
fn draw_menu(
    ...
    // Which menu is open lives in the layout, and the popup was built from
    // it: there is nothing to draw when nothing is open.
    ...
    for (index, cell) in popup.cells() {
        ...
        match menu {
            Menu::Zoom => {
                ...
                let (background, ink) =
                    button_ink(choice.active(view, image, layout.viewport), hover, theme);
