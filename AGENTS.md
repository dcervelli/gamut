...
| A picture in the README or `user-docs/` | not made here: the scripts that take them, one per picture, are in the `gamut-scripting` repository beside this one, and write into `user-docs/screenshots/`. Run the picture's script again after the interface changes rather than retouching the picture |
