...
bin/           release, and pkgbuild-sha which points the PKGBUILD at a published tag;
               screenshots/ holds one script per picture in user-docs/screenshots/,
               over lib.sh, which opens, drives and captures a window on Hyprland,
               and device.py, a uinput mouse and keyboard for the wheel, drags,
               and the keys that are bound by position
...
| A picture in the README or `user-docs/` | a script in `bin/screenshots/` over `lib.sh`: `open` the window, `keys` to drive it, `settle`, `shoot`, `close`; a recording is `record` and `cut` around the keys, then `gif`; the wheel and a drag are `wheel` and `drag`, after `cursor` has placed the pointer, at a point `fitted` works out from image coordinates; a key bound by position (`Shift+2`) is `press shift+2`. Run it again after the interface changes rather than retouching the picture |
