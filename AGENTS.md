...
cli.rs         argument parsing; usage() renders the key sections from app::input::ROWS at
               the default keymap, and the MOUSE section from the default gestures
settings.rs    the configuration file the user writes (Config, which the flags for the same
               panels win over) and the state file the program keeps (State, StateFile —
               read at startup, written in App::exiting)
app/           the event loop's state and winit handlers
  ...
  files.rs       Files: the file list, the read in flight, walks past broken files, the files taken
  ...
  filmstrip.rs   Filmstrip: the file list's state — the order in force, whether the list has fallen
                 out of it, the rows the strip was last drawn from and which are on screen
...
ui/            lays each frame's interface out with egui; no wgpu or winit imports
  ...
  filmstrip.rs   the file list: Sort, Direction and Order, which the menu offers; the panel down
                 the left — its head of the menu of sorts and the back and forward pair, and its
                 virtualized rows of thumbnails with the index and name over each
...
image/         the data model, nothing GPU
  ...
  resample.rs    a CPU box filter in the file's own encoding, for the thumbnails; and resize(), the
                 export's picture as shown made another size in linear light — area where it
...
| What something is called when the pointer rests on it | a `Tip` variant in `ui/tooltip.rs` and one `pass.tooltip(response, tip, enabled)` on the widget's response; `Namer::tooltip` in `app/input.rs` composes the words, from the keymap in force by way of `action_of` wherever a key does the same job, so a tooltip and `--help` cannot disagree. Words of its own go in `ui/tooltip.rs::words`, a zoom cell's in `ZoomChoice::describe`. When it opens and where it goes are egui's, tuned in `ui/style.rs` |
...
| A panel or overlay | a new `ui/<name>.rs` with a `show(pass, ui, ..)` that opens `panel::area` at the rectangle its own `panel()` works out through `panel::fit` from `pass.content`, and one call in `Pass::overlays`; the picture is `pass.current`, the device's grid `pass.grid`. An area that is `interactable` takes the pointer from the picture under it; `Order` is the height in the stack. A new color role goes in `theme/mod.rs` and, if a stock widget wears it, `ui/style.rs`. A panel that is part of the chrome instead — one the picture is fitted beside, as the file list is — is a flag of `chrome::Parts`, a rectangle `Chrome::new` works out, and an `egui::Panel` in `Pass::bars` given exactly that size |
...
| Anything a user installs — the desktop entry, the icon, a completion, the man page | `packaging/`; the man page is generated by `cli.rs::man()` from the same `OPTIONS` and `ROWS` as `--help`, so it is never edited directly, and the icon by `bin/icon`, which is where a change to it is made before running it again |
| An image format | `image/decode/<fmt>.rs` implementing `Decoder`, one line in `DECODERS`, a fixture in `test_images/` (see its README and `generate.sh`); a decoder that reads several formats answers `Decoder::format` with which one the leading bytes say, since that is what the info panel's "Read by" line and the file list's type show; a camera format is LibRaw's to read and `decode/raw.rs`'s to recognize, checked against a real file by `test_images/raw-samples/fetch.sh` |
...
| What the transport bar shows or does | `ui/transport.rs` for the bar and `App::transport` for what it is told; a press goes through `App::press` as `Control::{Play, StepBack, StepForward, Seek}`, and the keys through `App::step_frame` and `App::toggle_play` so the two cannot drift; `chrome::Parts` is where the bar takes its height |
| The file list: what a row shows, how the list is ordered, what back and forward do | `ui/filmstrip.rs` for the panel, and for `Sort` — a new one is a variant with its `label` and `describe`, in `ALL`, an arm of `order::rank`, and an arm of `Known::sorted_by` for what the row shows of it; `app/order.rs` for the permutation; `app/filmstrip.rs` for the rows and the stale flag; `App::apply_order` applies the order between reads and `App::poll_order` is when, `App::list_changed` and `App::facts_learned` are what marks it stale; `app/visited.rs` for the history and `App::visit` for a step along it; `App::press` for `Control::{Filmstrip, SortBy, SortDirection, Back, Forward, Thumb}` and `App::act` for `Command::FilmstripVisible`; `Panels::show_filmstrip` is whether it is up and `App::filmstrip_showing` whether it is on screen, which `chrome::Parts` gives its width by; the width is the slot `app/filmstrip.rs` holds between `SLOT_MIN` and `SLOT_MAX`, which a drag of `ui/filmstrip.rs::grip` asks for through `Command::FilmstripSlot` |
