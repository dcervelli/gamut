...
| What the info panel says about a file | `ui/info.rs` for the layout; the file's own facts are gathered in `app/mod.rs::file_facts`, its metadata in `image/exif.rs`, and its georeference in `image/geo.rs`. A raw's `Sensor` section, and the exposure of one whose EXIF says nothing, are what LibRaw read, in `decode/raw.rs::facts`; a raw container's EXIF block is found by `image/enclosed.rs` |
