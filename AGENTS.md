...
ui/            lays each frame's interface out with egui; no wgpu or winit imports
  ...
  filmstrip.rs   the file list: Section, Sort and Order, which the menus offer; the panel down
                 the left — its head of two menus and the back and forward pair, and its
...
| The file list: what a row shows, how the list is ordered, what a section is, what back and forward do | `ui/filmstrip.rs` for the panel, and for `Section` and `Sort` — a new one is a variant with its `label` and `describe`, in `ALL`, and an arm of `order::rank` or `Key::section`; `app/order.rs` for the permutation and the sections; `app/filmstrip.rs` for the rows and the stale flag; `App::apply_order` applies the order between reads and `App::poll_order` is when, `App::list_changed` and `App::facts_learned` are what marks it stale; `app/visited.rs` for the history and `App::visit` for a step along it; `App::press` for `Control::{Filmstrip, SectionBy, SortBy, Back, Forward, Thumb}` and `App::act` for `Command::FilmstripVisible`; `Panels::show_filmstrip` is whether it is up and `App::filmstrip_showing` whether it is on screen, which `chrome::Parts` gives its width by; the width is the slot `app/filmstrip.rs` holds between `SLOT_MIN` and `SLOT_MAX`, which a drag of `ui/filmstrip.rs::grip` asks for through `Command::FilmstripSlot` |
