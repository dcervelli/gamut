...
| A picture in the README or `user-docs/` | a script in `bin/screenshots/` over `lib.sh`: `open` the window, `keys` to drive it, `settle`, `shoot`, `close`; a recording is `record` and `cut` around the keys, then `gif`. Run it again after the interface changes rather than retouching the picture |
