...
| What the loupe magnifies, or where it goes | `ui/loupe.rs`: `RADIUS` and `MAGNIFICATION` for the circles, `place` for where the glass sits, `glass` for the draw; `App::loupe` for when it is up, from the toggle or `Pointer::secondary`, which `Command::Secondary` sets from the picture's response in `Pass::picture`; the cut and the replacing draw are `image_layer`'s loupe slot and `shaders/image.wgsl`'s `clip` |
