...
bin/           release, and pkgbuild-sha which points the PKGBUILD at a published tag;
               ...
               and mouse.py, a uinput mouse for the wheel and drags
...
| A picture in the README or `user-docs/` | a script in `bin/screenshots/` over `lib.sh`: `open` the window, `keys` to drive it, `settle`, `shoot`, `close`; a recording is `record` and `cut` around the keys, then `gif`; the wheel and a drag are `wheel` and `drag`, after `cursor` has placed the pointer, at a point `fitted` works out from image coordinates. Run it again after the interface changes rather than retouching the picture |
